...
//   const [allInternships, setAllInternships] = useState([]);
//   const navigate = useNavigate();
//   const { id } = useParams(); // Internship_ID

//   useEffect(() => {
//     // Fetch specific internship
...
//     // Fetch all internships for recommendations
...
//   const getModeColor = (mode) => {
//     switch (mode) {
//       case 'Remote': return 'bg-blue-50 text-blue-700 border border-blue-200';
//       case 'On-site': return 'bg-slate-50 text-slate-700 border border-slate-200';
//       case 'Hybrid': return 'bg-purple-50 text-purple-700 border border-purple-200';
//       default: return 'bg-slate-50 text-slate-700 border border-slate-200';
//     }
//   };
...
//         key={i}
//         size={16}
//         className={i < Math.floor(rating || 0) ? 'fill-amber-400 text-amber-400' : 'text-slate-300'}
//       />
//     ));
//   };
...
//   return (
//     <div className="min-h-screen bg-white">
//       {/* ... Keep your entire styling, navbar, animated background, tab structure ... */}
//       {/* Just replace all mock data usages with API data: */}
//       {/* For example: */}
//       <img src={internshipData.Company_Logo_URL || internshipData.logo} alt={internshipData.Company_Name} />
//       <h1>{internshipData.Internship_Title}</h1>
//       <span>{internshipData.Company_Name}</span>
//       <span>{internshipData.Sector}</span>
//       <span>{internshipData.Area_Field}</span>
//       <span className={`px-4 py-2 rounded-full text-sm font-semibold shadow-sm ${getModeColor(internshipData.Mode)}`}>
//         {internshipData.Mode}
//       </span>
//       <span>{internshipData.Duration_Months} months</span>
//       <span>{internshipData.Stipend_Amount ? `₹${internshipData.Stipend_Amount}` : 'Unpaid'}</span>
//       <span>{new Date(internshipData.Application_Deadline).toLocaleDateString()}</span>
...
//           <span>{rec.Internship_Title}</span>
//           <span className={`px-2 py-1 text-xs ${getModeColor(rec.Mode)}`}>{rec.Mode}</span>
//         </button>
//       ))}

//       {/* Required Skills */}
//       {internshipData.Required_Skills && internshipData.Required_Skills.map((skill, idx) => (
...
//       {/* Hiring Workflow
//       {internshipData.Hiring_Workflow && internshipData.Hiring_Workflow.map((step, index) => (
//         <div key={index}>{step}</div>
//       ))} */}

//       {/* Company Description */}
//       <p>{internshipData.Company_Description}</p>

//       {/* Contact */}
//       {internshipData.Requirement_Contact && (
//         <>
//           <p>{internshipData.Requirement_Contact.email}</p>
//           <p>{internshipData.Requirement_Contact.phone}</p>
//         </>
//       )}
//     </div>
//   );
